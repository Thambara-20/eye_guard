...
class StorageService {
  ...
  static const String _backgroundMonitoringKey =
  ...
  Future<void> saveLightReading(LightReading reading) async {
    ...
    final List<String> jsonReadings =
  ...
  Future<List<LightReading>> getReadingsInRange(
      DateTime start, DateTime end) async {
    final readings = await getLightReadings();

    return readings.where((reading) {
      return reading.timestamp.isAfter(start) &&
  ...
  // Save background monitoring preference
  Future<void> saveBackgroundMonitoring(bool enabled) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_backgroundMonitoringKey, enabled);
  }

  // Get background monitoring preference (defaults to true if not set)
  Future<bool> getBackgroundMonitoring() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_backgroundMonitoringKey) ?? true;
  }
}
